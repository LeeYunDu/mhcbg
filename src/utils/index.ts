...
export function getValue (data, field) {
  ...
  try {
    switch (true) {
      ...
      case !!field.transform:
        uValue = get(data, field.key, '') ? getDictValue(field.transform, get(data, field.key, '')) : '-'
        break
      case !!field.valueFormat:
        uValue = get(data, field.key, '') ? getDictValue(field.valueFormat, get(data, field.key, '')) : '-'
        break
  ...
  if (uValue == null || uValue == undefined || uValue == '' || uValue == 'null' || uValue == 'undefined') {
    uValue = '-'
  }
