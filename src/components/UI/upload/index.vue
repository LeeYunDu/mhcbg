  ...
  <div class="btnBox">
    ...
    <template v-else>
      <div class="upload-box">
        <div class="upload-gourp" @click="goUpload">
          <slot name="uploadBtn">
            <el-button
              ...
              :loading="isUploading"
            >
              <img :src="imgPath('app/icon_upload.png')" class="icon_up">
              <span>点击上传</span>
            </el-button>
          </slot>
        </div>
        ...
        <!-- 上传提示 -->
        <div v-if="isShowTip" class="upload_tip " style="margin-left:10px">
          <slot name="tips">
            <p v-if="tips" v-html="tips"></p>
            <p v-else>
              <img :src="imgPath('app/icon-tip.png')" class="icon_tip" alt="">
              请上传
              <template v-if="fileSize">
                大小不超过
                <b>{{ fileSize }}MB</b> ,
...
export default defineComponent({
  ...
  setup (props: any, ctx: SetupContext) {
      ...
      try {
        ...
        xhr.onreadystatechange = () => {
          if (xhr.status == 200) {
            ...
            res.success =  Number(res.code) === 200
            res.errMsg = res.msg || res.msg
            if (!res.success) {
          ...
          if (item.status && ['active'].indexOf(item.status) !== -1) {
            if (uploadSuccess) {
              ...
              if (findItem && findItem[urlKey.value]) {
                fileList.value[index][urlKey.value] = fileDomain + findItem[urlKey.value]
              }
            }
            ...
            if(uploadSuccess){
              delete fileList.value[index].file
              delete fileList.value[index].percentTimer
              delete fileList.value[index].percent
              delete fileList.value[index].status
            }
          }
        })
        changeVal()
    ...
    const changeVal = () => {
      let useData = cloneDeep(fileList.value).map((item:any)=>{
        return item
      })
      ctx.emit('update:modelValue', JSON.stringify(useData))
      ctx.emit('change', JSON.stringify(useData))
    }
